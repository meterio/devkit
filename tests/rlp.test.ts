...
describe('rlp', () => {
  ...
  it('blobKind encode', () => {
    ...
    expect(kind.data('0x1234567890', '').encode().toString('hex')).equal('1234567890');

    expect(() => {
      kind.data('0x1', '').encode();
    }, 'odd hex').to.throw();
    expect(() => {
      kind.data('0xxy', '').encode();
    }).to.throw();
    expect(() => {
      kind.data(1 as any, '').encode();
    }).to.throw();
  });
  ...
  it('numericKind encode', () => {
    ...
    expect(() => {
      kind.data('0x12345678123456780', '');
    }, 'exceed max bytes').to.throw();
    expect(() => {
      kind.data(2 ** 64, '');
    }, 'unsafe integer').to.throw();
  });
  it('numericKind decode', () => {
    ...
    expect(kind.buffer(Buffer.from([1, 2, 3, 4, 5, 6, 7, 8]), '').decode()).equal(
      '0x102030405060708'
    );

    expect(() => {
      kind.buffer(Buffer.alloc(9, 1), '').decode();
    }, 'exceeds max bytes').to.throw();
    expect(() => {
      kind.buffer(Buffer.from([0, 1]), '').decode();
    }, 'with leading zero').to.throw();
  });
  ...
  it('fixed blobKind encode', () => {
    ...
    expect(() => {
      kind.data('0x1234567z', '').encode();
    }).to.throw();
    expect(() => {
      kind.data('0x11', '').encode();
    }).to.throw();
    expect(() => {
      kind.data('0x1234567890', '').encode();
    }).to.throw();
    expect(() => {
      kind.data('0x1234567', 'odd hex').encode();
    }).to.throw();
    expect(() => {
      kind.data(1 as any, '').encode();
    }).to.throw();
    expect(() => {
      kind.data(null as any, '').encode();
    }).to.throw();
  });
  ...
  it('fixed blobKind decode', () => {
    ...
    expect(() => {
      kind.buffer(Buffer.alloc(2), '').decode();
    }).to.throw();
    expect(() => {
      kind.buffer(Buffer.alloc(0), '').decode();
    }).to.throw();
  });
